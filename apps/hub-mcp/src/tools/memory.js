 ...
 */
export function registerMemoryTools(server, env) {
    // memory.store — persist a memory for an agent
    server.tool('cortex.memory.store', 'Store a memory for an AI agent. Memories persist across sessions and can be recalled by semantic search. Use projectId + branch to scope memories to a specific branch.', {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: JSON.stringify({ stored: true, userId, projectId, branch, result }, null, 2),
                    },
                ],
            };
        }
    ...
    server.tool('cortex.memory.search', 'Search agent memories by semantic similarity. Use projectId + branch to search branch-specific knowledge with fallback to project-level and then agent-level memories.', {
        ...
        try {
            ...
            for (const userId of searchScopes) {
                ...
                const response = await fetch(`${env.MEM0_URL}/v1/memories/search/`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        query,
