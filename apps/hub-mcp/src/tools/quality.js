 ...
 */
export function registerQualityTools(server, env) {
    // quality.report — upload AWF gate checks
    server.tool('cortex.quality.report', 'Report the results of a Quality Gate check (e.g. Forgewright Phase/Gate checks, test outputs, lint records)', {
        ...
        try {
            ...
            if (!response.ok) {
                return {
                    content: [{ type: 'text', text: `Quality track failed: HTTP ${response.status}` }],
                    isError: true,
                };
            }
            return {
                content: [{ type: 'text', text: `Quality Report Logged: ${gate_name} (Passed: ${passed})` }],
            };
        }
        catch (error) {
            return {
                content: [{ type: 'text', text: `Quality API network error: ${String(error)}` }],
                isError: true,
            };
        }
    });
    // session.start — Start task context
    server.tool('cortex.session.start', 'Start a new execution session to track progress against product requirements.', {
        ...
        try {
            ...
            if (!response.ok)
                return { content: [{ type: 'text', text: `Fail: ${response.status}` }], isError: true };
            const data = await response.json();
            return { content: [{ type: 'text', text: `Tracking session started. Session ID: ${data.sessionId}` }] };
        }
        catch (error) {
